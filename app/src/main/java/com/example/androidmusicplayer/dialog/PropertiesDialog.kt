...
import com.example.androidmusicplayer.extension.canModifyEXIF
...
import com.example.androidmusicplayer.extension.getExifCameraModel
import com.example.androidmusicplayer.extension.getExifDateTaken
import com.example.androidmusicplayer.extension.getExifProperties
...
import com.example.androidmusicplayer.extension.isAudioSlow
import com.example.androidmusicplayer.extension.isPathOnInternalStorage
...
import com.example.androidmusicplayer.extension.md5
import com.example.androidmusicplayer.extension.removeValues
...
import com.example.androidmusicplayer.extension.sumByInt
...
class PropertiesDialog : BasePropertiesDialog {
        ...
        ensureBackgroundThread {
            ...
            if (!fileDirItem.isDirectory) {
                ...
                val exif = if (isNougatPlus() && mActivity.isPathOnOTG(fileDirItem.path)) {
                    ExifInterface((mActivity as BaseCustomActivity).getFileInputStreamSync(fileDirItem.path)!!)
    ...
    private fun addExifProperties(path: String, activity: Activity) {
        val exif = if (isNougatPlus() && activity.isPathOnOTG(path)) {
            ExifInterface((activity as BaseCustomActivity).getFileInputStreamSync(path)!!)
